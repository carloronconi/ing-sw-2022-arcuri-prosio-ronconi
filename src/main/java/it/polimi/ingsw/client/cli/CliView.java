...
public class CliView implements ViewInterface {
    ...
    public void chooseCloud() {
        ...
        String cloud = askUserInput("Choose a cloud:", s->{
            UUID id = converter.nameToId(s, ClientNameIdConverter.ConverterSetting.CLOUD);
        ...
        UUID uuid = converter.nameToId(cloud, ClientNameIdConverter.ConverterSetting.CLOUD);
    ...
    public void moveStudent() {
        ...
        if(isIsland){
            String textualIsland = askUserInput("Choose the island:", s->{
                return converter.nameToId(s, ClientNameIdConverter.ConverterSetting.ISLAND)!=null;
            });

            islandId = converter.nameToId(textualIsland, ClientNameIdConverter.ConverterSetting.ISLAND);
    ...
    public void gameOver(UUID winner) {
        if (winner == null) System.out.println("Game over!");
            ...
            String winnerNickname = converter.idToName(winner, ClientNameIdConverter.ConverterSetting.PLAYER);
    ...
    public void getCharacterSettings(AvailableCharacter forCharacter) {
        ...
        if (EffectWithIsland.class.isAssignableFrom(characterClass)){
            String text = askUserInput("Select an island for the character effect:", s->{
                UUID isl = converter.nameToId(s, ClientNameIdConverter.ConverterSetting.ISLAND);
                if (Witch.class.isAssignableFrom(characterClass)) return !gameState.getBanOnIslands().get(isl);
                return isl != null;
            });

            //island = UUID.fromString(text);
            island = converter.nameToId(text, ClientNameIdConverter.ConverterSetting.ISLAND);
        ...
        if (SwapperCharacter.class.isAssignableFrom(characterClass)) {
            ...
            EnumMap<PawnColor, Integer> entrance = gameState.getEntrances().get(converter.nameToId(finalNickname, ClientNameIdConverter.ConverterSetting.PLAYER));
            EnumMap<PawnColor, Integer> dining = gameState.getDiningRooms().get(converter.nameToId(finalNickname, ClientNameIdConverter.ConverterSetting.PLAYER));
    ...
    public void update(ModelEvent modelEvent) {
        if(modelEvent instanceof GameState){
            ...
            if(converter==null) converter = new ClientNameIdConverter(gameState);
            ...
            System.out.println(gameState.drawGameState(converter.nameToId(finalNickname, ClientNameIdConverter.ConverterSetting.PLAYER), initialIslandIds));
        }
