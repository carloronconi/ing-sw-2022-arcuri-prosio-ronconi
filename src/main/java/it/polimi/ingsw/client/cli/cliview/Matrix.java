...
public class Matrix implements Serializable {
    ...
    public Matrix(ClientNameIdConverter converter, HashMap<UUID, UUID> islandOwners, LinkedHashMap<UUID, TowerColor> colorPlayersTowers, LinkedHashMap<UUID, Boolean> banOnIslands, UUID motherNaturePosition, LinkedHashMap<UUID, Integer> islandsSize, ArrayList<UUID> initialIslandIds, LinkedHashMap<UUID, ArrayList<PawnColor>> islands){
        ...
        for (UUID uuid : initialIslandIds){
            ...
            if (islands.containsKey(uuid)){
                ...
                String islandName = converter.idToName(uuid, ClientNameIdConverter.ConverterSetting.ISLAND);
    ...
    /*public Matrix(ClientNameIdConverter converter, HashMap<UUID, UUID> islandOwners, LinkedHashMap<UUID, TowerColor> colorPlayersTowers, LinkedHashMap<UUID, Boolean> banOnIslands, UUID motherNaturePosition, LinkedHashMap<UUID, Integer> islandsSize, LinkedHashMap<UUID, ArrayList<PawnColor>> islands){
        ...
        for (UUID id : islands.keySet()){
            String name = converter.idToName(id, ClientNameIdConverter.ConverterSetting.ISLAND);
